...
public class WailsBridge {
    ...
    // Large binding responses (e.g. OpenNote with base64 assets) cannot safely
    // pass through evaluateJavascript as an escaped string — store and fetch instead.
    private final ConcurrentHashMap<String, byte[]> largeBindingPayloads = new ConcurrentHashMap<>();
    private static final int LARGE_BINDING_THRESHOLD = 96 * 1024;
    ...
    /**
     * Store a large binding response and return a short token for JS to fetch via
     * https://wails.localhost/__binding_payload__/{token}
     */
    public String storeLargeBindingPayload(String response) {
        if (response == null) {
            return null;
        }
        String token = UUID.randomUUID().toString().replace("-", "");
        largeBindingPayloads.put(token, response.getBytes(java.nio.charset.StandardCharsets.UTF_8));
        return token;
    }

    public boolean isLargeBindingPayload(String response) {
        return response != null && response.length() >= LARGE_BINDING_THRESHOLD;
    }

    @Nullable
    public byte[] takeLargeBindingPayload(String token) {
        if (token == null || token.isEmpty()) {
            return null;
        }
        return largeBindingPayloads.remove(token);
    }
