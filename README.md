...
当前应用版本 **3.0.0**，`.tnote` 格式版本 **7**，由后端和前端共同维护；结构变化需要显式迁移逻辑。
...
- **明文连接提示**：正式构建下连接非本机的 `ws://` 服务器（如局域网 IP）时，会弹一次"未加密连接可能被窃听"的警告。连接不会被阻断，局域网信任环境下照常使用；介意窃听可给服务端配 wss 反代。
- **房间失效的提示更明确**：邀请链接过期（房间被清理）后会提示"房间不可用"，需房主重新发起联机。旧版会静默"复活"一个空房间、文档看似丢失，实际是建了新房——此怪异行为已移除。
- **阅读视图富文本过滤**：打开 `.tnote` 或阅读协作者同步的内容时，富文本会先经白名单过滤（新增 `dompurify` 依赖）。正常笔记的样式、字体、加粗斜体、行内图片不受影响；极端复杂的大文档阅读渲染可能略增开销。
- **协作素材传输增加校验**：接收协作者分享的素材时会校验完整性（SHA-256、大小上限 256 MB、MIME 白名单）。正常传输无感；若校验失败素材会静默丢弃（界面表现为素材未到达），可请对方重新拖入。
- **素材链接只认内联数据**：素材中的 `dataUrl` 仅接受 `data:` 协议，外部 http 图片链接不再被自动加载，避免访问者被当作用户 IP 探测的信标。本地导入的素材不受影响。
- **邀请链接解析更严格**：`roomKey` 只从链接 `#` 片段读取，不再接受查询参数（`?roomKey=...`）形式的旧链接。新版客户端生成的邀请链接始终使用片段形式。
- **日志不再包含地址栏敏感信息**：应用日志中的页面地址会剥离 `#` 片段与查询串，邀请链接中的 roomKey 不会经日志落盘。
